...
bool ofQuickTimeGrabber::initGrabber(int w, int h){
	...
	//---------------------------------
		...
		if( !qtInitSeqGrabber() ){
			cout <<"ofQuickTimeGrabber: unable to initialize the seq grabber";
			return false;
		}
    ...
    nums[0] = kCFBooleanTrue; //CFNumberRef(0, kCFNumberIntType kCFNumberSInt32Type, &page);
    CFDictionaryRef ioProps = CFDictionaryCreate(0, 
                                              (const void**)keys, 
                                              (const void**)nums, 
                                              1, 
    ...
    CFStringRef key[1];
    CFDictionaryRef val[1];
    key[0] = kCVPixelBufferIOSurfacePropertiesKey;
    val[0] = ioProps;
    CFDictionaryRef bufferPros = CFDictionaryCreate(0, (const void**)key, (const void**)val, 1, &kCFTypeDictionaryKeyCallBacks,&kCFTypeDictionaryValueCallBacks);
    if(kCVReturnSuccess != CVPixelBufferCreate(NULL, w, h, kCVPixelFormatType_32BGRA, bufferPros, &pixelBuf)){
        printf("***** PIXEL BUFFER FAILED");
    }
    else {
		cout << "IOSURFACE " << getCurrentSurfaceID() << endl;
    ...
    pixels = new unsigned char[w*h*4];
		...
		#else
			QTNewGWorldFromPtr (&(videogworld), kCVPixelFormatType_32BGRA, &(videoRect), NULL, NULL, 0, pixels, 4 * w);
		    //QTNewGWorldFromPtr (&(videogworld), k32ARGBPixelFormat, &(videoRect), NULL, NULL, 0, pixels, 4 * w);
		#endif		
	...
	//---------------------------------
		...
		if (bGrabberInited == true){
			...
			if (bHavePixelsChanged){
                ...
                convertPixels(offscreenGWorldPixels, getPixels(), width, height);
				#endif
			}
		}
		...
		if (bGrabberInited == true){
			...
			if (bHavePixelsChanged == true){
                ...
                memcpy(CVPixelBufferGetBaseAddress(pixelBuf), pixels, CVPixelBufferGetBytesPerRow(pixelBuf)*CVPixelBufferGetHeight(pixelBuf));
...
IOSurfaceID ofQuickTimeGrabber::getCurrentSurfaceID(){
    return IOSurfaceGetID(CVPixelBufferGetIOSurface(pixelBuf));
}
...
float ofQuickTimeGrabber::getWidth(){
    return CVPixelBufferGetWidth(pixelBuf);
...
float ofQuickTimeGrabber::getHeight(){
    return CVPixelBufferGetHeight(pixelBuf);
...
void ofQuickTimeGrabber::videoSettings(void){
	...
	//---------------------------------
		...
		if (err != noErr){
			cout <<"ofQuickTimeGrabber: error in SGGetChannelBounds" << endl;
			return;
		}
		err = SGGetVideoRect (gVideoChannel, &curVideoRect);
		if (err != noErr){
			cout <<"ofQuickTimeGrabber: error in SGGetVideoRect" << endl;
			return;
		}
		...
		if (err != noErr){
			cout <<"ofQuickTimeGrabber: error in SGPause" << endl;
			return;
		}
			...
			if (result != noErr){
				cout <<"ofQuickTimeGrabber: error in  dialogue" << endl;
				return;
			}
...
bool ofQuickTimeGrabber::saveSettings(){
		...
		if ( err != noErr ){
			cout <<"ofQuickTimeGrabber: error getting camera settings %i" << err << endl;;
...
bool ofQuickTimeGrabber::loadSettings(){
   ...
   if (mySGVideoSettings){
      ...
      if ( err != noErr ) {
          cout <<"ofQuickTimeGrabber: error applying stored settings %i" << err << endl;
         return false;
      }
      DisposeUserData(mySGVideoSettings);
...
bool ofQuickTimeGrabber::qtInitSeqGrabber(){
		...
		if (bSgInited != true){
			...
			if (sgCompID == NULL){
//				cout <<"ofQuickTimeGrabber: findNextComponent did not return a valid component");
				return false;
			}
...
//				cout <<"ofQuickTimeGrabber: can't get default sequence grabber component");
				return false;
			}

			err = SGInitialize(gSeqGrabber);
			if (err != noErr) {
//				cout <<"ofQuickTimeGrabber: can't initialize sequence grabber component");
				return false;
			}

			err = SGSetDataRef(gSeqGrabber, 0, 0, seqGrabDontMakeMovie);
			if (err != noErr) {
//				cout <<"ofQuickTimeGrabber: can't set the destination data reference");
				return false;
			}
			...
			if (err != noErr) {
//				cout <<"ofQuickTimeGrabber: setting up the gworld");
				return false;
			}

			err = SGNewChannel(gSeqGrabber, VideoMediaType, &(gVideoChannel));
			if (err != noErr) {
//				cout <<"error: creating a channel.  Check if you have any qt capable cameras attached");
				return false;
			}
...
bool ofQuickTimeGrabber::qtSelectDevice(int deviceNumber, bool didWeChooseADevice){
	...
	if(numDevices == 0){
//		cout <<"ofQuickTimeGrabber: no catpure devices found");
		return false;
	}
...
//							//ofLog(OF_LOG_WARNING, "ofQuickTimeGrabber: unable to open device, trying next device");
			...
			//cout <<"(unavailable) device[%i] %s",  deviceCount, p2cstr(pascalName) );
			deviceCount++;
		}
	}
